...
#   - Ex: hexpm/elixir:1.15.7-erlang-26.1.2-debian-bullseye-20240904-slim
#
...
COPY --from=builder --chown=nobody:root /app/_build/${MIX_ENV}/rel/boba_talkie ./

USER nobody

# If using a startup script, copy and use it
# COPY rel/overlays/bin/server /app/bin/server
# CMD ["/app/bin/server"]

# Otherwise, start the Phoenix server directly
CMD ["/app/bin/boba_talkie", "start"]